...
export class ConnectionHandler {
  ...
  // O constructor agora só precisa da config
  ...
  public sendMessage(message: Buffer) {
    // ... método sendMessage continua igual ...
    ...
    if (this.socket instanceof net.Socket && this.socket.writable) {
      ...
      this.logger.warn('Socket não está pronto para escrita.');
    }
  }

  private startServer() {
    // ... método startServer continua igual ...
    ...
    server.on('error', (err) => { this.logger.error('Erro no servidor TCP:', err); });
    server.listen(this.config.port, this.config.host, () => { this.logger.log(`Servidor ouvindo na porta ${this.config.port}`); });
    this.socket = server;
  }

  private startClient() {
    // ... método startClient continua igual ...
    this.logger.log(`Iniciando como CLIENTE para ${this.config.host}:${this.config.port}...`);
    if (!this.config.host) {
      this.logger.error(`Host não definido para a conexão do tipo cliente: ${this.config.connectionId}`);
      return;
    }
    this.socket = new net.Socket();
    this.socket.connect(this.config.port, this.config.host, () => { this.logger.log('Cliente conectado com sucesso!'); });
    this.socket.on('close', () => { this.logger.warn('Conexão do cliente fechada. Tentando reconectar em 5s...'); });
    this.socket.on('error', (err) => { this.logger.error('Erro no cliente TCP:', err.message); });
  }
}